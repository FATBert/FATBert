...
from transformers import (WEIGHTS_NAME, get_linear_schedule_with_warmup, AdamW, RobertaConfig, RobertaForSequenceClassification, RobertaTokenizer)
from utils_v5 import (compute_metrics, convert_examples_to_features, output_modes, processors)
...
def main():
    ...
    setattr(args, "tokenizer_name", "")
    setattr(args,"cache_dir", "")
    setattr(args,"max_seq_length", 128)
    setattr(args,"do_train", False)
    setattr(args,"do_eval", False)
    setattr(args,"do_predict", False)
    setattr(args,"evaluate_during_training", False)
    setattr(args,"do_lower_case", False)

    setattr(args,"per_gpu_train_batch_size", 8)
    ...
    setattr(args,"num_train_epochs", 3.0)
    setattr(args,"max_steps", -1)
    setattr(args,"warmup_steps", 0)

    setattr(args,'logging_steps', 50)
    setattr(args,'save_steps', 50)
    setattr(args,"eval_all_checkpoints", False)
    setattr(args,"no_cuda", False)
    setattr(args,'overwrite_output_dir', False)
    setattr(args,'overwrite_cache', False)
    setattr(args,'seed', 42)

    setattr(args,'fp16', False)
    setattr(args,'fp16_opt_level', 'O1')
    setattr(args,"local_rank", -1) #-1
    setattr(args,'server_ip', '')
    setattr(args,'server_port', '')

    pretrained_model='microsoft/codebert-base'
    
    train_file = 'V5_C2_TREINAMENTO.txt'
    valid_file = 'V5_C2_VALIDAÇÃO.txt'
    test_file = 'TEST_CLASSES_DUVIDOSAS.txt'
    data_dir = 'C:/ProgramData/Anaconda3/envs/Codebert-Raquel/data/codesearch/'
    output_dir = 'C:/ProgramData/Anaconda3/envs/Codebert-Raquel/data/codesearch/'

    #Train settings
    '''
    setattr(args, "model_type", "roberta")
    setattr(args, "task_name", "person_classification")
    setattr(args, "do_train", True)
    setattr(args, "do_eval", True)
    setattr(args, "eval_all_checkpoints", True)
    setattr(args, "train_file", train_file)
    setattr(args, "dev_file", valid_file)
    setattr(args, "max_seq_length", 210)

    setattr(args, "per_gpu_train_batch_size", 10)
    setattr(args, "per_gpu_eval_batch_size", 10)
    setattr(args, "learning_rate", 1e-5)
    setattr(args, "num_train_epochs", 10)

    setattr(args, "gradient_accumulation_steps", 1)
    setattr(args, "overwrite_output_dir", True)
    setattr(args, "data_dir", data_dir)
    setattr(args, "output_dir", output_dir)
    setattr(args, "model_name_or_path", pretrained_model)
    setattr(args, "tokenizer_name", pretrained_model)
    '''
    #Test settings
    ...
    setattr(args, "test_file", 'batch_' + str(idx) + '.txt')
    setattr(args, "pred_model_dir", 'C:/ProgramData/Anaconda3/envs/Codebert-Raquel/data/codesearch/models/checkpoint-best/')
    setattr(args, "test_result_dir", 'C:/ProgramData/Anaconda3/envs/Codebert-Raquel/data/codesearch/test/results/' + str(idx) + '_batch_result.txt')
    setattr(args, "model_name_or_path", pretrained_model)
    setattr(args, "tokenizer_name", pretrained_model)
